...
@section('container')
    ...
    <form action="/berita-member/{{$beritas->id}}" method="POST" enctype="multipart/form-data">
    @method('PUT')    
    @csrf
        <div class="mb-3">
            ...
            @if($beritas->image)
            <img src="{{ asset('storage/'.$beritas->image) }}" class="image-thumbnail" alt="thumbnail" style="width:50%">
            @else
            <span class="badge badge-danger">Belum ada Thumbnail</span>
            @endif   
            <input type="file" id="image" name="image" class=" form-control @error('image') is-invalid @enderror" accepts=".png, .jpg, .jpeg">
        </div>
        @error('image')
                {{$message}}
            @enderror
